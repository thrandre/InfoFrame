...
export interface IEnumerable<TIn> {
    ...
    distinctBy: <TOut>(selector: ISelector<TIn, TOut>) => IEnumerable<TIn>;
    distinct: () => IEnumerable<TIn>;

    union: (target: IEnumerable<TIn>) => IEnumerable<TIn>;
...
export interface IAggregatorFunction<TIn, TOut> {
...
export function fromArray<T>(arr: T[]): IEnumerable<T> {
    return Enumerable.fromArray(arr);
}

export function fromObject(obj: { [id: string]: any }): IEnumerable<IKeyValuePair<string, any>> {
    return Enumerable.fromObject(obj);
}

export default class Enumerable {
    static fromArray<TVal>(arr: TVal[]): IEnumerable<TVal> {
        return new EnumerableCore(arr);
    }

    static fromObject(obj: { [id: string]: any }): IEnumerable<IKeyValuePair<string, any>> {
        var pairs: KeyValuePair<string, any>[] = [];
        for (var key in obj) {
...
class AggregationAggregator<TIn, TOut> implements IAggregator<TIn, TOut> {
    ...
    constructor(
        private _aggregatorFunction: IAggregatorFunction<TIn, TOut>) { }
}
...
class GroupingAggregator<TIn, TOut> implements IAggregator<TIn, IEnumerable<Grouping<TIn, TOut>>> {
    ...
    getResult(): IEnumerable<Grouping<TIn, TOut>> {
        return Enumerable.fromArray(this._storage);
    }
...
export class EnumerableCore<TIn> implements IEnumerable<TIn> {
    ...
    protected aggr<TOut>(aggFunc: IAggregatorFunction<TIn, TOut>): TOut {
        return new Iterator(this.getEnumerator()).aggregate(i => new IterationResult(i, false), new AggregationAggregator(aggFunc));
    }

    protected iterate<TOut>(iterator: IIteratorKernel<TIn, TOut>, aggregator: IAggregator<TOut, IEnumerable<TOut>>): IEnumerable<TOut> {
        return new Iterator(this.getEnumerator()).filter(iterator, aggregator);
    }

    protected group<TOut, TOut2>(iterator: IIteratorKernel<TIn, TOut>, aggregator: IAggregator<TOut, IEnumerable<Grouping<TIn, TOut2>>>): IEnumerable<Grouping<TIn, TOut2>> {
        return new Iterator(this.getEnumerator()).filter(iterator, aggregator);
    }

    protected filter<TOut>(iterator: IIteratorKernel<TIn, TOut>): IEnumerable<TOut> {
        return this.iterate(iterator, new FilterAggregator<TOut>());
    }

    protected sort<TOut>(selector: ISelector<TIn, TOut>, order: SortOrder): IEnumerable<TIn> {
    ...
    first(predicate?: IPredicate<TIn>): TIn {
        ...
        if (!result) {
            throw Error("Invalid operation. Sequence containts no elements.");
        }
    ...
    aggregate<TOut>(aggFunc: IAggregatorFunction<TIn, TOut>, seed: TOut): TOut {
        return this.aggr<TOut>((sum, next) => {
    ...
    sum(selector?: ISelector<TIn, number>): number {
        if (!selector) {
            selector = (i) => <number><any>i;
        }
        return this.aggregate<number>((sum, next) => sum + selector(next), 0);
    }
    ...
    distinctBy<TOut>(selector: ISelector<TIn, TOut>): IEnumerable<TIn> {
        return this.group(i => new IterationResult(i, false), new GroupingAggregator(selector)).select(g => g.firstOrDefault());
    }

    distinct(): IEnumerable<TIn> {
        var selector: ISelector<TIn, TIn> = i => i;
        return this.distinctBy(selector);
    }

    union(target: IEnumerable<TIn>): IEnumerable<TIn> {
        return new List(this.toArray().concat(target.toArray())).distinct();
    }
